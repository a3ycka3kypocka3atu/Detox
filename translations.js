const translations = {
    // ════════════ NAVBAR ════════════
    "nav_science": {
        "ru": "Наука о Биофлоре",
        "ua": "Наука про Біофлору",
        "cz": "Věda o Bioflóře",
        "en": "Bioflora Science"
    },
    "nav_naturo": {
        "ru": "Натуропатия",
        "ua": "Натуропатія",
        "cz": "Naturopatie",
        "en": "Naturopathy"
    },
    "nav_life": {
        "ru": "Сферы Жизни",
        "ua": "Сфери Життя",
        "cz": "Sféry Života",
        "en": "Spheres of Life"
    },
    "nav_tools": {
        "ru": "Инструменты",
        "ua": "Інструменти",
        "cz": "Nástroje",
        "en": "Tools"
    },
    "theme_toggle": {
        "ru": "Сменить тему",
        "ua": "Змінити тему",
        "cz": "Změnit motiv",
        "en": "Toggle Theme"
    },

    // ════════════ MAIN HERO (PANEL 2) ════════════
    "hero_badge": {
        "ru": "Натуропатический детокс",
        "ua": "Натуропатичний детокс",
        "cz": "Naturopatický detox",
        "en": "Naturopathic Detox"
    },
    "hero_title": {
        "ru": "Станьте архитектором собственного здоровья.",
        "ua": "Станьте архітектором власного здоров'я.",
        "cz": "Staňte se architektem vlastního zdraví.",
        "en": "Become the architect of your own health."
    },
    "hero_sub": {
        "ru": "Мы верим, что ваше тело — это совершенная система, способная к самовосстановлению. Нужно лишь дать ему правильные инструменты.",
        "ua": "Ми віримо, що ваше тіло — це досконала система, здатна до самовідновлення. Потрібно лише дати йому правильні інструменти.",
        "cz": "Věříme, že vaše tělo je dokonalý systém schopný samoobnovy. Stačí mu jen poskytnout správné nástroje.",
        "en": "We believe your body is a perfect system capable of self-healing. You just need to provide it with the right tools."
    },
    "hero_desc": {
        "ru": "Детокс — это не экзекуция и не разовая акция. Это регулярная гигиена, которую необходимо проводить раз в полгода. Мы опираемся на глубокую научную экспертизу (нейробиологию и микробиом) в сочетании с мудростью натуропатии и аюрведы.",
        "ua": "Детокс — це не екзекуція і не разова акція. Це регулярна гігієна, яку необхідно проводити раз на півроку. Ми спираємося на глибоку наукову експертизу (нейробіологію та мікробіом) у поєднанні з мудрістю натуропатії та аюрведи.",
        "cz": "Detox není mučení ani jednorázová akce. Je to pravidelná hygiena, kterou je nutné provádět jednou za půl roku. Opíráme se o hlubokou vědeckou odbornost (neurobiologii a mikrobiom) v kombinaci s moudrostí naturopatie a ájurvédy.",
        "en": "Detox is not an execution or a one-time event. It is regular hygiene that should be performed every six months. We rely on deep scientific expertise (neurobiology and the microbiome) combined with the wisdom of naturopathy and Ayurveda."
    },
    "env_h2": {
        "ru": "Современный мир — это вызов для вашей внутренней экосистемы.",
        "ua": "Сучасний світ — це виклик для вашої внутрішньої екосистеми.",
        "cz": "Moderní svět je výzvou pro váš vnitřní ekosystém.",
        "en": "The modern world is a challenge for your internal ecosystem."
    },
    "env_p1": {
        "ru": "Хронический стресс, обработанная еда, нарушение сна, экология — всё это бьет по вашему внутреннему миру. Многие не подозревают, что в их кишечнике живут «незваные гости» — паразиты разных видов, которые есть практически у каждого.",
        "ua": "Хронічний стрес, оброблена їжа, порушення сну, екологія — все це б'є по вашому внутрішньому світу. Багато хто не підозрює, що в їхньому кишечнику живуть «непрохані гості» — паразити різних видів, які є практично у кожного.",
        "cz": "Chronický stres, zpracované potraviny, poruchy spánku, ekologie — to vše útočí na váš vnitřní svět. Mnozí netuší, že v jejich střevech žijí „nezvaní hosté“ — paraziti různých druhů, které má prakticky každý.",
        "en": "Chronic stress, processed food, sleep deprivation, and pollution all take a toll on your inner world. Many do not realize that 'uninvited guests' — various types of parasites — live in their gut, affecting almost everyone."
    },
    "env_p2": {
        "ru": "Они влияют на наши желания (тяга к сладкому), уровень энергии и даже настроение. Мы объясняем биохимию, а не пугаем. Цель — очистить внутреннюю среду, чтобы создать условия, где болезнь просто не может существовать.",
        "ua": "Вони впливають на наші бажання (тяга до солодкого), рівень енергії і навіть настрій. Ми пояснюємо біохімію, а не лякаємо. Мета — очистити внутрішнє середовище, щоб створити умови, де хвороба просто не може існувати.",
        "cz": "Ovlivňují naše touhy (chuť na sladké), hladinu energie a dokonce i náladu. Vysvětlujeme biochemii, nestrašíme. Cílem je vyčistit vnitřní prostředí a vytvořit podmínky, kde nemoc prostě nemůže existovat.",
        "en": "They influence our cravings (sugar addiction), energy levels, and even our mood. We explain biochemistry rather than use scare tactics. The goal is to cleanse the internal environment to create conditions where disease simply cannot exist."
    },
    "symp_h2": {
        "ru": "Ваше тело говорит с вами. Вы слышите его?",
        "ua": "Ваше тіло говорить з вами. Ви чуєте його?",
        "cz": "Vaše tělo k vám mluví. Slyšíte ho?",
        "en": "Your body is speaking to you. Are you listening?"
    },
    "symp_1": {
        "ru": "«Туман в мозге» и сложность с концентрацией",
        "ua": "«Туман в мозку» та складнощі з концентрацією",
        "cz": "„Mlha v mozku“ a potíže se soustředěním",
        "en": "Brain fog and difficulty concentrating"
    },
    "symp_2": {
        "ru": "Постоянная усталость, даже после сна",
        "ua": "Постійна втома, навіть після сну",
        "cz": "Neustálá únava, dokonce i po spánku",
        "en": "Constant fatigue, even after waking up"
    },
    "symp_3": {
        "ru": "Проблемы с кожей (акне, высыпания)",
        "ua": "Проблеми зі шкірою (акне, висипання)",
        "cz": "Problémy s pletí (akné, vyrážky)",
        "en": "Skin issues (acne, breakouts)"
    },
    "symp_4": {
        "ru": "Эмоциональные качели и потеря творческого ресурса",
        "ua": "Емоційні гойдалки та втрата творчого ресурсу",
        "cz": "Emoční výkyvy a ztráta tvůrčího zdroje",
        "en": "Emotional swings and loss of creative resources"
    },
    "symp_5": {
        "ru": "Плохая память и ощущение «потерянности»",
        "ua": "Погана пам'ять та відчуття «загубленості»",
        "cz": "Špatná paměť a pocit „ztracenosti“",
        "en": "Poor memory and feeling 'lost'"
    },
    "symp_p": {
        "ru": "Это не ваша вина и не норма. Это сигнал о перегрузке. Мы приглашаем вас на прекрасный путь возвращения к себе настоящему — энергичному, креативному и продуктивному.",
        "ua": "Це не ваша вина і не норма. Це сигнал про перевантаження. Ми запрошуємо вас на прекрасний шлях повернення до себе справжнього — енергійного, креативного та продуктивного.",
        "cz": "Není to vaše vina a není to norma. Je to signál přetížení. Zveme vás na krásnou cestu návratu k vašemu skutečnému já — energickému, kreativnímu a produktivnímu.",
        "en": "This is not your fault, nor is it normal. It is a sign of systemic overload. We invite you on a beautiful journey back to your true self — energetic, creative, and productive."
    },
    "meth_h2": {
        "ru": "Экологичный путь к чистоте: Три вектора восстановления.",
        "ua": "Екологічний шлях до чистоти: Три вектори відновлення.",
        "cz": "Ekologická cesta k čistotě: Tři vektory obnovy.",
        "en": "The eco-friendly path to purity: Three vectors of recovery."
    },
    "meth_p": {
        "ru": "Мы не используем агрессивную химию. Наш метод — это мягкое очищение через:",
        "ua": "Ми не використовуємо агресивну хімію. Наш метод — це м'яке очищення через:",
        "cz": "Nepoužíváme agresivní chemii. Naší metodou je jemné čištění prostřednictvím:",
        "en": "We do not use harsh chemicals. Our method is a gentle cleanse through:"
    },
    "meth_1_h": {
        "ru": "Горькие травы и настойка ПВЖМ",
        "ua": "Гіркі трави та настоянка ПВВМ",
        "cz": "Hořké byliny a tinktura",
        "en": "Bitter Herbs & Wax Moth Tincture"
    },
    "meth_1_p": {
        "ru": "Для нейтрализации патогенов и растворения их защитных оболочек.",
        "ua": "Для нейтралізації патогенів та розчинення їх захисних оболонок.",
        "cz": "Pro neutralizaci patogenů a rozpuštění jejich ochranných obalů.",
        "en": "To neutralize pathogens and dissolve their protective biofilms."
    },
    "meth_2_h": {
        "ru": "Специальный чай и живые ферменты",
        "ua": "Спеціальний чай та живі ферменти",
        "cz": "Speciální čaj a živé enzymy",
        "en": "Special Tea & Live Enzymes"
    },
    "meth_2_p": {
        "ru": "Для восстановления слизистой и работы лимфы.",
        "ua": "Для відновлення слизової та роботи лімфи.",
        "cz": "Pro obnovu sliznice a lymfatického systému.",
        "en": "To restore the mucosal lining and support lymphatic drainage."
    },
    "meth_3_h": {
        "ru": "Сорбенты (байндеры)",
        "ua": "Сорбенти (байндери)",
        "cz": "Sorbenty (vazače)",
        "en": "Sorbents (Binders)"
    },
    "meth_3_p": {
        "ru": "Безопасное выведение токсинов как «губкой».",
        "ua": "Безпечне виведення токсинів як «губкою».",
        "cz": "Bezpečné odstraňování toxinů jako „houbou“.",
        "en": "Safely extracting toxins like a natural 'sponge'."
    },
    "btn_more_microbiome": {
        "ru": "Узнать больше о Микробиоме",
        "ua": "Дізнатися більше про Мікробіом",
        "cz": "Zjistit více o Mikrobiomu",
        "en": "Learn more about the Microbiome"
    },
    "btn_view_kit": {
        "ru": "Посмотреть состав набора",
        "ua": "Подивитися склад набору",
        "cz": "Prohlédnout složení sady",
        "en": "View the Kit Components"
    },
    "story_h2": {
        "ru": "От личного поиска к созданию сообщества.",
        "ua": "Від особистого пошуку до створення спільноти.",
        "cz": "Od osobního hledání k vytvoření komunity.",
        "en": "From personal quest to building a community."
    },
    "story_p": {
        "ru": "Личный опыт борьбы за здоровье привел к открытию основ натуропатии. Первые результаты вдохновили делиться знаниями с друзьями, близкими, а затем и создавать целую экосистему Santiago Wellness Hub.",
        "ua": "Особистий досвід боротьби за здоров'я призвів до відкриття основ натуропатії. Перші результати надихнули ділитися знаннями з друзями, близькими, а згодом і створювати цілу екосистему Santiago Wellness Hub.",
        "cz": "Osobní zkušenost boje o zdraví vedla k objevení základů naturopatie. První výsledky nás inspirovaly ke sdílení znalostí s přáteli, rodinou a následně k vytvoření celého ekosystému Santiago Wellness Hub.",
        "en": "A personal battle for health led to the discovery of naturopathic principles. The initial results inspired us to share this knowledge with friends and family, eventually creating the entire Santiago Wellness Hub ecosystem."
    },
    "testim_1": {
        "ru": "«Вернулась ясность ума. Я наконец-то могу сосредоточиться на творчестве без постоянного тумана.»",
        "ua": "«Повернулася ясність розуму. Я нарешті можу зосередитися на творчості без постійного туману.»",
        "cz": "„Vrátila se mi jasná mysl. Konečně se mohu soustředit na tvorbu bez neustálé mlhy.“",
        "en": "«Mental clarity has returned. I can finally focus on creativity without constant brain fog.»"
    },
    "testim_2": {
        "ru": "«Исчезла тяга к сахару. Это казалось невозможным, но через 2 недели я просто перестала хотеть сладкое.»",
        "ua": "«Зникла тяга до цукру. Це здавалося неможливим, але через 2 тижні я просто перестала хотіти солодке.»",
        "cz": "„Zmizela chuť na cukr. Zdálo se to nemožné, ale po 2 týdnech jsem prostě přestala mít chuť na sladké.“",
        "en": "«My sugar cravings vanished. It seemed impossible, but after 2 weeks, I simply stopped wanting sweets.»"
    },
    "testim_3": {
        "ru": "«Появились силы на новые проекты. Энергия, которую я не чувствовал годами.»",
        "ua": "«З'явилися сили на нові проєкти. Енергія, яку я не відчував роками.»",
        "cz": "„Objevila se síla pro nové projekty. Energie, kterou jsem necítil celá léta.“",
        "en": "«I found the strength for new projects. Energy I haven't felt in years.»"
    },
    "testim_author": {
        "ru": "— Участник программы",
        "ua": "— Учасник програми",
        "cz": "— Účastník programu",
        "en": "— Program Participant"
    },

    // ════════════ SCIENCE (PANEL 0) ════════════
    "sc_badge": {
        "ru": "Фундаментальные знания",
        "ua": "Фундаментальні знання",
        "cz": "Základní znalosti",
        "en": "Fundamental Knowledge"
    },
    "sc_title": {
        "ru": "Вселенная внутри нас",
        "ua": "Всесвіт всередині нас",
        "cz": "Vesmír uvnitř nás",
        "en": "The Universe Within Us"
    },
    "sc_sub": {
        "ru": "Наука о Микробиоме — фундамент здоровья, доказательная база и исследования",
        "ua": "Наука про Мікробіом — фундамент здоров'я, доказова база та дослідження",
        "cz": "Věda o Mikrobiomu – základ zdraví, důkazní báze a výzkum",
        "en": "Microbiome Science — the foundation of health, evidence base, and research"
    },
    "sc_btn_pdf1": {
        "ru": "Скачать полное руководство «Биофлора» (PDF)",
        "ua": "Завантажити повний посібник «Біофлора» (PDF)",
        "cz": "Stáhnout kompletního průvodce „Bioflóra“ (PDF)",
        "en": "Download the Complete 'Bioflora' Guide (PDF)"
    },
    "sc_h2_1": {
        "ru": "Вы не одни. Познакомьтесь с триллионами своих союзников.",
        "ua": "Ви не самі. Познайомтеся з трильйонами своїх союзників.",
        "cz": "Nejste sami. Seznamte se s biliony vašich spojenců.",
        "en": "You are not alone. Meet your trillions of allies."
    },
    "sc_p_1": {
        "ru": "Мы привыкли воспринимать свое тело как единый механизм, но наука говорит о другом. Мы разберем основы нашей внутренней экологии. Здесь мы раскрываем понятие «голобионта» — как человек и триллионы бактерий, грибов и вирусов живут в симбиозе. От баланса этой системы зависит ваш иммунитет, энергия и даже настроение.",
        "ua": "Ми звикли сприймати своє тіло як єдиний механізм, але наука говорить про інше. Ми розберемо основи нашої внутрішньої екології. Тут ми розкриваємо поняття «голобіонта» — як людина і трильйони бактерій, грибів і вірусів живуть у симбіозі. Від балансу цієї системи залежить ваш імунітет, енергія і навіть настрій.",
        "cz": "Jsme zvyklí vnímat své tělo jako jeden mechanismus, ale věda říká něco jiného. Rozebereme si základy naší vnitřní ekologie. Zde vysvětlujeme koncept „holobionta“ — jak lidé a biliony bakterií, hub a virů žijí v symbióze. Od rovnováhy tohoto systému se odvíjí vaše imunita, energie a dokonce i nálada.",
        "en": "We are used to perceiving our body as a single mechanism, but science says otherwise. We will explore the basics of our internal ecology. Here we unpack the concept of the 'holobiont' — how humans and trillions of bacteria, fungi, and viruses live in symbiosis. Your immunity, energy, and even your mood depend on the balance of this system."
    },
    "sc_h2_2": {
        "ru": "Война миров: Симбионты против Патогенов",
        "ua": "Війна світів: Симбіонти проти Патогенів",
        "cz": "Válka světů: Symbionti proti Patogenům",
        "en": "War of the Worlds: Symbionts vs. Pathogens"
    },
    "sc_good_guys": {
        "ru": "Хорошие парни",
        "ua": "Хороші хлопці",
        "cz": "Ti dobří",
        "en": "The Good Guys"
    },
    "sc_good_p": {
        "ru": "Такие симбионты, как Treponema, Prevotella и Bifidobacterium, критически важны для переваривания пищи и обучения нашего иммунитета.",
        "ua": "Такі симбіонти, як Treponema, Prevotella та Bifidobacterium, критично важливі для перетравлення їжі та навчання нашого імунітету.",
        "cz": "Takoví symbionti jako Treponema, Prevotella a Bifidobacterium jsou kriticky důležití pro trávení potravy a trénink naší imunity.",
        "en": "Symbionts such as Treponema, Prevotella, and Bifidobacterium are critical for digesting food and training our immune system."
    },
    "sc_bad_guys": {
        "ru": "Плохие парни",
        "ua": "Погані хлопці",
        "cz": "Ti špatní",
        "en": "The Bad Guys"
    },
    "sc_bad_p": {
        "ru": "Влияние антибиотиков, стерильных городских условий, стресса и западного стиля питания ведет к вымиранию полезных видов. В результате появляются патогенные грибки (кандида), паразиты и бактерии.",
        "ua": "Вплив антибіотиків, стерильних міських умов, стресу та західного стилю харчування веде до вимирання корисних видів. У результаті з'являються патогенні грибки (кандида), паразити та бактерії.",
        "cz": "Vliv antibiotik, sterilního městského prostředí, stresu a západního stylu stravování vede k vymírání prospěšných druhů. Výsledkem je vznik patogenních plísní (kandida), parazitů a bakterií.",
        "en": "The impact of antibiotics, sterile urban conditions, stress, and the Western diet leads to the extinction of beneficial species. As a result, pathogenic fungi (candida), parasites, and harmful bacteria emerge."
    },
    "sc_h2_3": {
        "ru": "Как ваш живот управляет вашей головой",
        "ua": "Як ваш живіт керує вашою головою",
        "cz": "Jak vaše břicho řídí vaši hlavu",
        "en": "How Your Gut Controls Your Brain"
    },
    "sc_p_3": {
        "ru": "Это самый захватывающий блок о том, как пищеварение диктует правила нашему мозгу.",
        "ua": "Це найбільш захоплюючий блок про те, як травлення диктує правила нашому мозку.",
        "cz": "Toto je nejvíce fascinující část o tom, jak trávení diktuje pravidla našemu mozku.",
        "en": "This is the most fascinating section on how digestion dictates the rules to our brain."
    },
    "sc_feat_1_h": {
        "ru": "Блуждающий нерв (Vagus Nerve)",
        "ua": "Блукаючий нерв (Vagus Nerve)",
        "cz": "Bloudivý nerv (Nervus Vagus)",
        "en": "The Vagus Nerve"
    },
    "sc_feat_1_p": {
        "ru": "Главная информационная магистраль, которая передает сигналы от бактерий напрямую в мозг.",
        "ua": "Головна інформаційна магістраль, яка передає сигнали від бактерій безпосередньо в мозок.",
        "cz": "Hlavní informační dálnice, která přenáší signály od bakterií přímo do mozku.",
        "en": "The main information highway that transmits signals from bacteria directly to the brain."
    },
    "sc_feat_2_h": {
        "ru": "Фабрика счастья",
        "ua": "Фабрика щастя",
        "cz": "Továrna na štěstí",
        "en": "The Happiness Factory"
    },
    "sc_feat_2_p": {
        "ru": "90-95% гормона счастья (серотонина) производится в кишечнике при помощи бактерий.",
        "ua": "90-95% гормону щастя (серотоніну) виробляється в кишечнику за допомогою бактерій.",
        "cz": "90-95 % hormonu štěstí (serotoninu) se produkuje ve střevech pomocí bakterií.",
        "en": "90-95% of the happiness hormone (serotonin) is produced in the gut with the help of bacteria."
    },
    "sc_feat_3_h": {
        "ru": "Замкнутый круг",
        "ua": "Замкнене коло",
        "cz": "Začarovaný kruh",
        "en": "The Vicious Cycle"
    },
    "sc_feat_3_p": {
        "ru": "Хронический стресс «парализует» пищеварение и разрушает микрофлору. Воспаление в кишечнике вызывает нейровоспаление, тревожность, депрессию и «туман в голове».",
        "ua": "Хронічний стрес «паралізує» травлення і руйнує мікрофлору. Запалення в кишечнику викликає нейрозапалення, тривожність, депресію та «туман в голові».",
        "cz": "Chronický stres „ochromuje“ trávení a ničí mikroflóru. Zánět ve střevech způsobuje neurozánět, úzkost, deprese a „mlhu v hlavě“.",
        "en": "Chronic stress 'paralyzes' digestion and destroys microflora. Gut inflammation causes neuroinflammation, anxiety, depression, and 'brain fog'."
    },
    "sc_h2_4": {
        "ru": "Мы — то, что едят наши бактерии",
        "ua": "Ми — те, що їдять наші бактерії",
        "cz": "Jsme to, co jedí naše bakterie",
        "en": "We Are What Our Bacteria Eat"
    },
    "sc_p_4": {
        "ru": "Здесь мы обсуждаем, как пища формирует наш внутренний мир.",
        "ua": "Тут ми обговорюємо, як їжа формує наш внутрішній світ.",
        "cz": "Zde probíráme, jak strava formuje náš vnitřní svět.",
        "en": "Here we discuss how food shapes our inner world."
    },
    "sc_food_bad_h": {
        "ru": "Что разрушает флору",
        "ua": "Що руйнує флору",
        "cz": "Co ničí flóru",
        "en": "What Destroys the Flora"
    },
    "sc_food_bad_p": {
        "ru": "Избыток рафинированного сахара, животных жиров и отсутствие клетчатки убивает разнообразие флоры и кормит патогенов.",
        "ua": "Надлишок рафінованого цукру, тваринних жирів і відсутність клітковини вбиває різноманітність флори та годує патогенів.",
        "cz": "Nadbytek rafinovaného cukru, živočišných tuků a nedostatek vlákniny zabíjí rozmanitost flóry a krmí patogeny.",
        "en": "An excess of refined sugar, animal fats, and a lack of fiber kills the diversity of the flora and feeds pathogens."
    },
    "sc_food_good_h": {
        "ru": "Что лечит",
        "ua": "Що лікує",
        "cz": "Co léčí",
        "en": "What Heals"
    },
    "sc_food_good_p": {
        "ru": "Сложные углеводы, пищевые волокна и растительная пища. Особенно мощно работают ферментированные продукты: кефир, квашеная капуста, комбуча и сыворотка.",
        "ua": "Складні вуглеводи, харчові волокна та рослинна їжа. Особливо потужно працюють ферментовані продукти: кефір, квашена капуста, комбуча та сироватка.",
        "cz": "Komplexní sacharidy, dietní vláknina a rostlinná strava. Obzvláště silně fungují fermentované potraviny: kefír, kysané zelí, kombucha a syrovátka.",
        "en": "Complex carbohydrates, dietary fiber, and plant-based foods. Fermented foods work particularly well: kefir, sauerkraut, kombucha, and whey."
    },
    "sc_h2_5": {
        "ru": "Как вернуть порядок? Клинический подход к очищению.",
        "ua": "Як повернути порядок? Клінічний підхід до очищення.",
        "cz": "Jak obnovit řád? Klinický přístup k čištění.",
        "en": "How to Restore Order? The Clinical Approach to Cleansing."
    },
    "sc_p_5": {
        "ru": "В медицине это называется не просто «чисткой», а строгим клиническим протоколом восстановления среды. Мы используем концепцию «5R»:",
        "ua": "У медицині це називається не просто «чисткою», а суворим клінічним протоколом відновлення середовища. Ми використовуємо концепцію «5R»:",
        "cz": "V medicíně se to nenazývá jen „čištěním“, ale přísným klinickým protokolem obnovy prostředí. Využíváme koncept „5R“:",
        "en": "In medicine, this isn't just called a 'cleanse', but a strict clinical protocol for environmental restoration. We utilize the '5R' framework:"
    },
    "sc_5r_1_h": {
        "ru": "Remove (Прополка)",
        "ua": "Remove (Прополка)",
        "cz": "Remove (Odstranění)",
        "en": "Remove"
    },
    "sc_5r_1_p": {
        "ru": "Убрать пищевой мусор и патогенов с помощью растительных антимикробных средств (полынь, гвоздика, черный орех).",
        "ua": "Прибрати харчове сміття та патогенів за допомогою рослинних антимікробних засобів (полин, гвоздика, чорний горіх).",
        "cz": "Odstranění potravinového odpadu a patogenů pomocí rostlinných antimikrobiálních látek (pelyněk, hřebíček, ořešák černý).",
        "en": "Eliminate junk food and pathogens using plant-based antimicrobials (wormwood, cloves, black walnut)."
    },
    "sc_5r_2_h": {
        "ru": "Replace (Замещение)",
        "ua": "Replace (Заміщення)",
        "cz": "Replace (Nahrazení)",
        "en": "Replace"
    },
    "sc_5r_2_p": {
        "ru": "Помощь желудку через ферменты и горечи для нормального переваривания.",
        "ua": "Допомога шлунку через ферменти та гіркоти для нормального перетравлення.",
        "cz": "Podpora žaludku pomocí enzymů a hořčin pro normální trávení.",
        "en": "Supporting the stomach with enzymes and bitters for proper digestion."
    },
    "sc_5r_3_h": {
        "ru": "Reinoculate (Заселение)",
        "ua": "Reinoculate (Заселення)",
        "cz": "Reinoculate (Znovuosídlení)",
        "en": "Reinoculate"
    },
    "sc_5r_3_p": {
        "ru": "Правильно ввести пробиотики, чтобы они прижились в среде.",
        "ua": "Правильно ввести пробіотики, щоб вони прижилися в середовищі.",
        "cz": "Správné zavedení probiotik, aby se v prostředí uchytila.",
        "en": "Properly introducing probiotics so they can colonize the environment."
    },
    "sc_5r_4_h": {
        "ru": "Repair (Заживление)",
        "ua": "Repair (Загоєння)",
        "cz": "Repair (Zhojení)",
        "en": "Repair"
    },
    "sc_5r_4_p": {
        "ru": "Восстановление слизистой кишечника с помощью костного бульона, витаминов и L-глютамина.",
        "ua": "Відновлення слизової кишечника за допомогою кісткового бульйону, вітамінів та L-глютаміну.",
        "cz": "Obnova střevní sliznice pomocí vývaru z kostí, vitamínů a L-glutaminu.",
        "en": "Healing the gut lining with bone broth, vitamins, and L-glutamine."
    },
    "sc_h2_6": {
        "ru": "Экология повседневности: Сохраняем результат",
        "ua": "Екологія повсякденності: Зберігаємо результат",
        "cz": "Ekologie všedního dne: Udržujeme výsledky",
        "en": "Everyday Ecology: Maintaining the Results"
    },
    "sc_p_6": {
        "ru": "Практические привычки для поддержания достигнутого результата.",
        "ua": "Практичні звички для підтримання досягнутого результату.",
        "cz": "Praktické návyky pro udržení dosaženého výsledku.",
        "en": "Practical habits to sustain the achieved results."
    },
    "sc_feat_4_h": {
        "ru": "Циркадное голодание",
        "ua": "Циркадне голодування",
        "cz": "Cirkadiánní půst",
        "en": "Circadian Fasting"
    },
    "sc_feat_4_p": {
        "ru": "Почему нужно не есть минимум 12 часов ночью? Это включает «внутреннего дворника», который выметает остатки пищи и бактерий.",
        "ua": "Чому потрібно не їсти мінімум 12 годин вночі? Це включає «внутрішнього двірника», який вимітає залишки їжі та бактерій.",
        "cz": "Proč je potřeba nejíst v noci minimálně 12 hodin? Zapíná to „vnitřního uklízeče“, který vymetá zbytky potravy a bakterií.",
        "en": "Why should you fast for at least 12 hours overnight? It activates the 'internal sweeper' that clears out food debris and bacteria."
    },
    "sc_feat_5_h": {
        "ru": "Стимуляция блуждающего нерва",
        "ua": "Стимуляція блукаючого нерва",
        "cz": "Stimulace bloudivého nervu",
        "en": "Vagus Nerve Stimulation"
    },
    "sc_feat_5_p": {
        "ru": "Дыхательные практики, йога и работа с парасимпатической нервной системой позволяют включить режим «отдыхай и переваривай».",
        "ua": "Дихальні практики, йога та робота з парасимпатичною нервовою системою дозволяють включити режим «відпочивай і перетравлюй».",
        "cz": "Dechová cvičení, jóga a práce s parasympatickým nervovým systémem umožňují zapnout režim „odpočívej a trav“.",
        "en": "Breathwork, yoga, and engaging the parasympathetic nervous system activate the 'rest and digest' mode."
    },
    "sc_cta_h2": {
        "ru": "Изучите свой микромир в деталях",
        "ua": "Вивчіть свій мікросвіт у деталях",
        "cz": "Prozkoumejte svůj mikrosvět v detailech",
        "en": "Explore Your Microworld in Detail"
    },
    "sc_cta_p": {
        "ru": "Эта страница — лишь верхушка айсберга. Мы собрали всю фундаментальную информацию о пробиотиках, питании, протоколе восстановления и работе блуждающего нерва в один красивый и понятный документ.",
        "ua": "Ця сторінка — лише верхівка айсберга. Ми зібрали всю фундаментальну інформацію про пробіотики, харчування, протокол відновлення та роботу блукаючого нерва в один красивий і зрозумілий документ.",
        "cz": "Tato stránka je jen špičkou ledovce. Shromáždili jsme všechny základní informace o probiotikách, výživě, protokolu obnovy a fungování bloudivého nervu do jednoho krásného a srozumitelného dokumentu.",
        "en": "This page is just the tip of the iceberg. We have compiled all the fundamental information on probiotics, nutrition, the recovery protocol, and the vagus nerve into one beautiful, easy-to-read document."
    },
    "sc_btn_pdf2": {
        "ru": "Скачать полное руководство по Биофлоре (PDF)",
        "ua": "Завантажити повний посібник з Біофлори (PDF)",
        "cz": "Stáhnout kompletního průvodce Bioflórou (PDF)",
        "en": "Download the Complete Bioflora Guide (PDF)"
    },
    "sc_hint": {
        "ru": "Сохраните на телефон, изучите в удобное время и обязательно поделитесь этим файлом с близкими — эти знания меняют качество жизни.",
        "ua": "Збережіть на телефон, вивчіть у зручний час і обов'язково поділіться цим файлом з близькими — ці знання змінюють якість життя.",
        "cz": "Uložte si ho do telefonu, nastudujte si ho ve vhodnou dobu a určitě ho sdílejte se svými blízkými — tyto znalosti mění kvalitu života.",
        "en": "Save it to your phone, study it at your convenience, and be sure to share this file with your loved ones — this knowledge is life-changing."
    },
    "sc_base_h2": {
        "ru": "Научная база: Исследования и статьи",
        "ua": "Наукова база: Дослідження та статті",
        "cz": "Vědecká základna: Výzkum a články",
        "en": "Scientific Base: Research & Articles"
    },
    "sc_base_p": {
        "ru": "Доказательный фундамент проекта — подробные описания экспериментов, ссылки на международные научные исследования и разбор биохимии компонентов.",
        "ua": "Доказовий фундамент проєкту — докладні описи експериментів, посилання на міжнародні наукові дослідження та розбір біохімії компонентів.",
        "cz": "Důkazní základ projektu — podrobné popisy experimentů, odkazy na mezinárodní vědecký výzkum a analýza biochemie komponent.",
        "en": "The evidence-based foundation of the project — detailed descriptions of experiments, links to international scientific studies, and biochemistry breakdowns."
    },
    "art_1_h": {
        "ru": "Влияние микробиома на нейротрансмиттеры",
        "ua": "Вплив мікробіома на нейротрансмітери",
        "cz": "Vliv mikrobiomu na neurotransmitery",
        "en": "Microbiome Influence on Neurotransmitters"
    },
    "art_1_p": {
        "ru": "Как кишечные бактерии производят 90-95% серотонина и управляют нашим настроением через ось «кишечник-мозг».",
        "ua": "Як кишкові бактерії виробляють 90-95% серотоніну і керують нашим настроєм через вісь «кишечник-мозок».",
        "cz": "Jak střevní bakterie produkují 90-95 % serotoninu a řídí naši náladu prostřednictvím osy „střevo-mozek“.",
        "en": "How gut bacteria produce 90-95% of serotonin and control our mood via the gut-brain axis."
    },
    "art_2_h": {
        "ru": "Биохимия антипаразитарных трав",
        "ua": "Біохімія антипаразитарних трав",
        "cz": "Biochemie antiparazitárních bylin",
        "en": "Biochemistry of Anti-Parasitic Herbs"
    },
    "art_2_p": {
        "ru": "Научный разбор механизмов действия полыни, пижмы и черного ореха на патогенные организмы.",
        "ua": "Науковий розбір механізмів дії полину, пижма та чорного горіха на патогенні організми.",
        "cz": "Vědecký rozbor mechanismů účinku pelyňku, vratiče a ořešáku černého na patogenní organismy.",
        "en": "A scientific breakdown of how wormwood, tansy, and black walnut act against pathogenic organisms."
    },
    "art_3_h": {
        "ru": "Блуждающий нерв и пищеварение",
        "ua": "Блукаючий нерв і травлення",
        "cz": "Bloudivý nerv a trávení",
        "en": "The Vagus Nerve and Digestion"
    },
    "art_3_p": {
        "ru": "Vagus Nerve — главная информационная магистраль между кишечником и мозгом. Как его стимуляция улучшает здоровье.",
        "ua": "Vagus Nerve — головна інформаційна магістраль між кишечником і мозком. Як його стимуляція покращує здоров'я.",
        "cz": "Vagus Nerve — hlavní informační dálnice mezi střevem a mozkem. Jak jeho stimulace zlepšuje zdraví.",
        "en": "The Vagus Nerve is the main information highway between the gut and the brain. How stimulating it improves health."
    },
    "art_4_h": {
        "ru": "Ферментированные продукты и флора",
        "ua": "Ферментовані продукти та флора",
        "cz": "Fermentované potraviny a flóra",
        "en": "Fermented Foods and Flora"
    },
    "art_4_p": {
        "ru": "Почему кефир, квашеная капуста и комбуча работают лучше многих таблеток — роль диких дрожжей и лактобактерий.",
        "ua": "Чому кефір, квашена капуста і комбуча працюють краще за багато пігулок — роль диких дріжджів і лактобактерій.",
        "cz": "Proč kefír, kysané zelí a kombucha fungují lépe než mnoho pilulek — role divokých kvasinek a laktobacilů.",
        "en": "Why kefir, sauerkraut, and kombucha often work better than pills — the role of wild yeast and lactobacilli."
    },
    "art_5_h": {
        "ru": "Клинический протокол 5R",
        "ua": "Клінічний протокол 5R",
        "cz": "Klinický protokol 5R",
        "en": "The 5R Clinical Protocol"
    },
    "art_5_p": {
        "ru": "Remove, Replace, Reinoculate, Repair — научный подход к восстановлению внутренней среды. Подробный разбор каждого этапа.",
        "ua": "Remove, Replace, Reinoculate, Repair — науковий підхід до відновлення внутрішнього середовища. Докладний розбір кожного етапу.",
        "cz": "Remove, Replace, Reinoculate, Repair — vědecký přístup k obnově vnitřního prostředí. Podrobný rozbor každé fáze.",
        "en": "Remove, Replace, Reinoculate, Repair — the scientific approach to restoring the internal environment. A detailed breakdown of each stage."
    },
    "art_6_h": {
        "ru": "Сорбенты: механизм действия",
        "ua": "Сорбенти: механізм дії",
        "cz": "Sorbenty: mechanismus účinku",
        "en": "Sorbents: Mechanism of Action"
    },
    "art_6_p": {
        "ru": "Как активированный уголь, глина и лен работают как природная губка, связывая и выводя токсины мертвых бактерий.",
        "ua": "Як активоване вугілля, глина та льон працюють як природна губка, зв'язуючи та виводячи токсини мертвих бактерій.",
        "cz": "Jak aktivní uhlí, jíl a len fungují jako přírodní houba, která váže a odstraňuje toxiny mrtvých bakterií.",
        "en": "How activated charcoal, clay, and flax act as a natural sponge, binding and eliminating toxins from dead bacteria."
    },

    // ════════════ NATUROPATHY (PANEL 1) ════════════
    "nat_badge": {
        "ru": "Древняя мудрость + Современная наука",
        "ua": "Стародавня мудрість + Сучасна наука",
        "cz": "Starověká moudrost + Moderní věda",
        "en": "Ancient Wisdom + Modern Science"
    },
    "nat_title": {
        "ru": "Натуропатия и Аюрведа",
        "ua": "Натуропатія та Аюрведа",
        "cz": "Naturopatie a Ájurvéda",
        "en": "Naturopathy and Ayurveda"
    },
    "nat_sub": {
        "ru": "Тысячелетние знания о гармонии тела и разума, подкрепленные современными исследованиями. Мы берем лучшее из обеих традиций.",
        "ua": "Тисячолітні знання про гармонію тіла і розуму, підкріплені сучасними дослідженнями. Ми беремо найкраще з обох традицій.",
        "cz": "Tisícileté poznatky o harmonii těla a mysli, podložené moderními výzkumy. Bereme to nejlepší z obou tradic.",
        "en": "Millennia of knowledge on body-mind harmony, backed by modern research. We take the best from both traditions."
    },
    "nat_h2_1": {
        "ru": "Что такое натуропатия?",
        "ua": "Що таке натуропатія?",
        "cz": "Co je to naturopatie?",
        "en": "What is Naturopathy?"
    },
    "nat_p_1": {
        "ru": "Натуропатия — это система оздоровления, основанная на вере в целительную силу природы (Vis Medicatrix Naturae). Она не борется с симптомами, а ищет и устраняет первопричину дисбаланса. Организм обладает врожденной способностью к самоисцелению — задача натуропата состоит в том, чтобы создать для этого правильные условия.",
        "ua": "Натуропатія — це система оздоровлення, заснована на вірі в цілющу силу природи (Vis Medicatrix Naturae). Вона не бореться з симптомами, а шукає і усуває першопричину дисбалансу. Організм має вроджену здатність до самозцілення — завдання натуропата полягає в тому, щоб створити для цього правильні умови.",
        "cz": "Naturopatie je systém péče o zdraví založený na víře v léčivou sílu přírody (Vis Medicatrix Naturae). Nebojuje s příznaky, ale hledá a odstraňuje hlavní příčinu nerovnováhy. Tělo má vrozenou schopnost sebeuzdravování — úkolem naturopata je vytvořit k tomu správné podmínky.",
        "en": "Naturopathy is a healing system based on the belief in the healing power of nature (Vis Medicatrix Naturae). It does not suppress symptoms; rather, it seeks to eliminate the root cause of imbalance. The body has an innate ability to heal itself — the naturopath's job is to create the right conditions for this to happen."
    },
    "nat_feat_1_h": {
        "ru": "Принцип «Не навреди»",
        "ua": "Принцип «Не нашкодь»",
        "cz": "Princip „Především neškodit“",
        "en": "The 'Do No Harm' Principle"
    },
    "nat_feat_1_p": {
        "ru": "Использование мягких, неинвазивных методов. Приоритет натуральных средств перед синтетическими.",
        "ua": "Використання м'яких, неінвазивних методів. Пріоритет натуральних засобів перед синтетичними.",
        "cz": "Použití šetrných, neinvazivních metod. Upřednostňování přírodních prostředků před syntetickými.",
        "en": "Using gentle, non-invasive methods. Prioritizing natural remedies over synthetic ones."
    },
    "nat_feat_2_h": {
        "ru": "Лечение причины, а не симптома",
        "ua": "Лікування причини, а не симптому",
        "cz": "Léčba příčiny, nikoli příznaku",
        "en": "Treating the Cause, Not the Symptom"
    },
    "nat_feat_2_p": {
        "ru": "Головная боль — это не дефицит обезболивающего. Натуропатия ищет корень: обезвоживание, токсины, стресс или дефицит нутриентов.",
        "ua": "Головний біль — це не дефіцит знеболювального. Натуропатія шукає корінь: зневоднення, токсини, стрес або дефіцит нутрієнтів.",
        "cz": "Bolest hlavy není nedostatkem léků proti bolesti. Naturopatie hledá kořen: dehydratace, toxiny, stres nebo nedostatek živin.",
        "en": "A headache is not a deficiency of painkillers. Naturopathy looks for the root: dehydration, toxins, stress, or nutrient deficiencies."
    },
    "nat_feat_3_h": {
        "ru": "Человек как целостная система",
        "ua": "Людина як цілісна система",
        "cz": "Člověk jako celostní systém",
        "en": "Holistic Approach"
    },
    "nat_feat_3_p": {
        "ru": "Тело, разум и дух — единое целое. Нельзя лечить желудок, игнорируя состояние психики и образ жизни.",
        "ua": "Тіло, розум і дух — єдине ціле. Не можна лікувати шлунок, ігноруючи стан психіки та спосіб життя.",
        "cz": "Tělo, mysl a duch tvoří jeden celek. Nelze léčit žaludek a ignorovat duševní stav a životní styl.",
        "en": "Mind, body, and spirit are one entity. You cannot treat the stomach while ignoring mental health and lifestyle."
    },
    "nat_h2_2": {
        "ru": "Аюрведа: наука о жизни",
        "ua": "Аюрведа: наука про життя",
        "cz": "Ájurvéda: věda o životě",
        "en": "Ayurveda: The Science of Life"
    },
    "nat_p_2": {
        "ru": "Аюрведа (санскр. «знание жизни») — одна из древнейших медицинских систем мира, зародившаяся более 5000 лет назад в Индии. Она рассматривает здоровье как баланс трех фундаментальных энергий — дош.",
        "ua": "Аюрведа (санскр. «знання життя») — одна з найдавніших медичних систем світу, що зародилася понад 5000 років тому в Індії. Вона розглядає здоров'я як баланс трьох фундаментальних енергій — дош.",
        "cz": "Ájurvéda (sanskrt. „věda o životě“) je jedním z nejstarších lékařských systémů na světě, který vznikl v Indii před více než 5000 lety. Zdraví vnímá jako rovnováhu tří základních energií — dóš.",
        "en": "Ayurveda (Sanskrit for 'knowledge of life') is one of the world's oldest medical systems, originating over 5,000 years ago in India. It views health as the balance of three fundamental energies — doshas."
    },
    "nat_dosha_vata_h": { "ru": "Вата", "ua": "Вата", "cz": "Váta", "en": "Vata" },
    "nat_dosha_vata_p": {
        "ru": "Воздух и эфир. Движение, дыхание, нервная система. Дисбаланс → тревожность, сухость, бессонница.",
        "ua": "Повітря та ефір. Рух, дихання, нервова система. Дисбаланс → тривожність, сухість, безсоння.",
        "cz": "Vzduch a éter. Pohyb, dýchání, nervový systém. Nerovnováha → úzkost, suchost, nespavost.",
        "en": "Air and ether. Movement, breath, nervous system. Imbalance → anxiety, dryness, insomnia."
    },
    "nat_dosha_pitta_h": { "ru": "Питта", "ua": "Пітта", "cz": "Pitta", "en": "Pitta" },
    "nat_dosha_pitta_p": {
        "ru": "Огонь и вода. Пищеварение, метаболизм, интеллект. Дисбаланс → воспаления, раздражительность, кожные проблемы.",
        "ua": "Вогонь і вода. Травлення, метаболізм, інтелект. Дисбаланс → запалення, дратівливість, шкірні проблеми.",
        "cz": "Oheň a voda. Trávení, metabolismus, intelekt. Nerovnováha → záněty, podrážděnost, kožní problémy.",
        "en": "Fire and water. Digestion, metabolism, intellect. Imbalance → inflammation, irritability, skin issues."
    },
    "nat_dosha_kapha_h": { "ru": "Капха", "ua": "Капха", "cz": "Kapha", "en": "Kapha" },
    "nat_dosha_kapha_p": {
        "ru": "Земля и вода. Структура, стабильность, иммунитет. Дисбаланс → вялость, набор веса, застойные процессы.",
        "ua": "Земля і вода. Структура, стабільність, імунітет. Дисбаланс → млявість, набір ваги, застійні процеси.",
        "cz": "Země a voda. Struktura, stabilita, imunita. Nerovnováha → letargie, přibírání na váze, stagnující procesy.",
        "en": "Earth and water. Structure, stability, immunity. Imbalance → lethargy, weight gain, congestion."
    },
    "nat_h2_3": {
        "ru": "Детокс в традиционных системах",
        "ua": "Детокс у традиційних системах",
        "cz": "Detox v tradičních systémech",
        "en": "Detox in Traditional Systems"
    },
    "nat_p_3": {
        "ru": "Очищение тела — центральная практика во всех древних медицинских традициях. В аюрведе это называется Панчакарма (пять действий очищения), в натуропатии — элиминационная диета с последующим восстановлением.",
        "ua": "Очищення тіла — центральна практика у всіх стародавніх медичних традиціях. В аюрведі це називається Панчакарма (п'ять дій очищення), у натуропатії — елімінаційна дієта з подальшим відновленням.",
        "cz": "Očista těla je ústřední praxí ve všech starověkých lékařských tradicích. V ájurvédě se tomu říká Pančakarma (pět očistných akcí), v naturopatii — eliminační dieta s následnou rekonvalescencí.",
        "en": "Cleansing the body is a central practice in all ancient medical traditions. In Ayurveda, this is called Panchakarma (five cleansing actions); in naturopathy, it is an elimination diet followed by restoration."
    },
    "nat_insight": {
        "ru": "Наш протокол Santiago объединяет эти подходы: мягкая элиминация патогенов (натуропатия) + восстановление баланса дош через питание и травы (аюрведа) + научно обоснованный протокол 5R (доказательная медицина).",
        "ua": "Наш протокол Santiago об'єднує ці підходи: м'яка елімінація патогенів (натуропатія) + відновлення балансу дош через харчування і трави (аюрведа) + науково обґрунтований протокол 5R (доказова медицина).",
        "cz": "Náš protokol Santiago spojuje tyto přístupy: jemná eliminace patogenů (naturopatie) + obnova rovnováhy dóš pomocí stravy a bylin (ájurvéda) + vědecky podložený protokol 5R (medicína založená na důkazech).",
        "en": "Our Santiago Protocol integrates these approaches: gentle pathogen elimination (naturopathy) + restoring dosha balance through diet and herbs (Ayurveda) + the scientifically validated 5R protocol (evidence-based medicine)."
    },
    "nat_h2_4": {
        "ru": "Ключевые практики оздоровления",
        "ua": "Ключові практики оздоровлення",
        "cz": "Klíčové postupy ozdravění",
        "en": "Key Healing Practices"
    },
    "nat_prac_1_h": {
        "ru": "Диначарья (Режим дня)",
        "ua": "Діначар'я (Режим дня)",
        "cz": "Dinacharya (Denní režim)",
        "en": "Dinacharya (Daily Routine)"
    },
    "nat_prac_1_p": {
        "ru": "Просыпаться с рассветом, есть основной приём пищи в полдень (когда огонь пищеварения максимален), засыпать до 22:00.",
        "ua": "Прокидатися зі світанком, їсти основний прийом їжі опівдні (коли вогонь травлення максимальний), засинати до 22:00.",
        "cz": "Probouzet se za úsvitu, mít hlavní jídlo v poledne (kdy je trávicí oheň maximální), usínat před 22:00.",
        "en": "Waking up with the dawn, having the main meal at noon (when the digestive fire is strongest), and sleeping before 10 PM."
    },
    "nat_prac_2_h": {
        "ru": "Агни (Пищеварительный огонь)",
        "ua": "Агні (Травний вогонь)",
        "cz": "Agni (Trávicí oheň)",
        "en": "Agni (Digestive Fire)"
    },
    "nat_prac_2_p": {
        "ru": "Здоровье начинается с пищеварения. Слабый агни = непереваренная пища (ама) = токсины. Специи, тёплая вода и правильный режим усиливают агни.",
        "ua": "Здоров'я починається з травлення. Слабкий агні = неперетравлена їжа (ама) = токсини. Спеції, тепла вода та правильний режим посилюють агні.",
        "cz": "Zdraví začíná trávením. Slabé agni = nestrávená potrava (ama) = toxiny. Koření, teplá voda a správný režim posilují agni.",
        "en": "Health begins with digestion. Weak Agni = undigested food (Ama) = toxins. Spices, warm water, and a proper routine strengthen Agni."
    },
    "nat_prac_3_h": {
        "ru": "Расаяна (Омоложение)",
        "ua": "Расаяна (Омолодження)",
        "cz": "Rasajana (Omlazení)",
        "en": "Rasayana (Rejuvenation)"
    },
    "nat_prac_3_p": {
        "ru": "После очищения тело готово к восстановлению. Адаптогены (ашваганда, шатавари), питательные масла и регенеративные практики.",
        "ua": "Після очищення тіло готове до відновлення. Адаптогени (ашваганда, шатаварі), живильні масла та регенеративні практики.",
        "cz": "Po vyčištění je tělo připraveno k regeneraci. Adaptogeny (ašvaganda, šatavari), výživné oleje a regenerační praktiky.",
        "en": "After cleansing, the body is ready for restoration. Adaptogens (ashwagandha, shatavari), nourishing oils, and regenerative practices."
    },
    "nat_prac_4_h": {
        "ru": "Пранаяма (Дыхание)",
        "ua": "Пранаяма (Дихання)",
        "cz": "Pránájáma (Dýchání)",
        "en": "Pranayama (Breathwork)"
    },
    "nat_prac_4_p": {
        "ru": "Контролируемое дыхание стимулирует блуждающий нерв, снижает кортизол и активирует парасимпатическую нервную систему.",
        "ua": "Контрольоване дихання стимулює блукаючий нерв, знижує кортизол і активує парасимпатичну нервову систему.",
        "cz": "Kontrolované dýchání stimuluje bloudivý nerv, snižuje kortizol a aktivuje parasympatický nervový systém.",
        "en": "Controlled breathing stimulates the vagus nerve, lowers cortisol, and activates the parasympathetic nervous system."
    },
    "nat_h2_5": {
        "ru": "Почему это работает сегодня",
        "ua": "Чому це працює сьогодні",
        "cz": "Proč to dnes funguje",
        "en": "Why It Works Today"
    },
    "nat_p_5": {
        "ru": "Современная наука подтверждает то, что аюрведа знала тысячелетиями: хронический стресс разрушает пищеварение, токсическая нагрузка подавляет иммунитет, а режим дня напрямую влияет на гормональный фон и нейропластичность мозга.",
        "ua": "Сучасна наука підтверджує те, що аюрведа знала тисячоліттями: хронічний стрес руйнує травлення, токсичне навантаження пригнічує імунітет, а режим дня безпосередньо впливає на гормональний фон і нейропластичність мозку.",
        "cz": "Moderní věda potvrzuje to, co ájurvéda znala po tisíciletí: chronický stres ničí trávení, toxická zátěž potlačuje imunitu a denní režim přímo ovlivňuje hormonální rovnováhu a neuroplasticitu mozku.",
        "en": "Modern science confirms what Ayurveda has known for millennia: chronic stress destroys digestion, toxic load suppresses immunity, and a daily routine directly impacts hormone balance and brain neuroplasticity."
    },
    "nat_sci_1_h": {
        "ru": "Что подтвердила наука",
        "ua": "Що підтвердила наука",
        "cz": "Co potvrdila věda",
        "en": "What Science Confirmed"
    },
    "nat_sci_1_p": {
        "ru": "Антимикробные свойства горьких трав. Роль пищеварительных ферментов. Влияние циркадных ритмов на метаболизм. Ось «кишечник-мозг».",
        "ua": "Антимікробні властивості гірких трав. Роль травних ферментів. Вплив циркадних ритмів на метаболізм. Вісь «кишечник-мозок».",
        "cz": "Antimikrobiální vlastnosti hořkých bylin. Role trávicích enzymů. Vliv cirkadiánních rytmů na metabolismus. Osa „střevo-mozek“.",
        "en": "Antimicrobial properties of bitter herbs. The role of digestive enzymes. Circadian rhythm impact on metabolism. The gut-brain axis."
    },
    "nat_sci_2_h": {
        "ru": "Что мы используем",
        "ua": "Що ми використовуємо",
        "cz": "Co používáme",
        "en": "What We Use"
    },
    "nat_sci_2_p": {
        "ru": "Аюрведические принципы питания по дошам. Натуропатический подход к элиминации. Научный протокол восстановления микробиома.",
        "ua": "Аюрведичні принципи харчування по дошах. Натуропатичний підхід до елімінації. Науковий протокол відновлення мікробіома.",
        "cz": "Ájurvédské principy výživy podle dóš. Naturopatický přístup k eliminaci. Vědecký protokol pro obnovu mikrobiomu.",
        "en": "Ayurvedic dosha-based nutrition. Naturopathic elimination approaches. A scientific microbiome restoration protocol."
    },

    // ════════════ SPHERES OF LIFE (PANEL 3) ════════════
    "life_badge": {
        "ru": "Путь к полноценной жизни",
        "ua": "Шлях до повноцінного життя",
        "cz": "Cesta k plnohodnotnému životu",
        "en": "The Path to a Fulfilling Life"
    },
    "life_title": {
        "ru": "Жизнь может быть искусством.",
        "ua": "Життя може бути мистецтвом.",
        "cz": "Život může být umění.",
        "en": "Life Can Be an Art."
    },
    "life_sub": {
        "ru": "Вы готовы стать творцом? Мы забыли, что развиваться, творить, строить отношения и достигать целей можно не через усталость и выгорание, а через искреннюю радость от процесса.",
        "ua": "Ви готові стати творцем? Ми забули, що розвиватися, творити, будувати відносини і досягати цілей можна не через втому і вигорання, а через щиру радість від процесу.",
        "cz": "Jste připraveni stát se tvůrcem? Zapomněli jsme, že rozvíjet se, tvořit, budovat vztahy a dosahovat cílů lze ne přes únavu a vyhoření, ale s upřímnou radostí z procesu.",
        "en": "Are you ready to become a creator? We have forgotten that growing, creating, building relationships, and achieving goals can be done not through exhaustion and burnout, but through genuine joy in the process."
    },
    "life_large_p": {
        "ru": "Спорт, музыка, путешествия, любимое дело, финансы и глубокие отношения — всё это делает нас лучше. И самое главное — это доступно каждому, это весело и дает колоссальный заряд энергии, а не забирает её.",
        "ua": "Спорт, музика, подорожі, улюблена справа, фінанси та глибокі відносини — все це робить нас кращими. І найголовніше — це доступно кожному, це весело і дає колосальний заряд енергії, а не забирає її.",
        "cz": "Sport, hudba, cestování, oblíbená činnost, finance a hluboké vztahy — to vše nás dělá lepšími. A to nejdůležitější — je to dostupné všem, je to zábavné a dodává to obrovskou dávku energie, místo aby ji bralo.",
        "en": "Sports, music, travel, passion projects, finances, and deep relationships — all these make us better. Most importantly, it is accessible to everyone, it is fun, and it gives you a massive energy boost instead of draining it."
    },
    "life_h2_1": {
        "ru": "Где взять искру?",
        "ua": "Де взяти іскру?",
        "cz": "Kde vzít jiskru?",
        "en": "Where to Find the Spark?"
    },
    "life_p_1": {
        "ru": "Мы понимаем, что человеку часто нужен старт. Мощный импульс и подзарядка, чтобы просто начать, а главное — не бросить, продолжать улучшать себя и становиться тем человеком, которым хочется быть. Трудно быть продуктивным, когда нет базовых сил.",
        "ua": "Ми розуміємо, що людині часто потрібен старт. Потужний імпульс і підзарядка, щоб просто почати, а головне — не кинути, продовжувати покращувати себе і ставати тією людиною, якою хочеться бути. Важко бути продуктивним, коли немає базових сил.",
        "cz": "Chápeme, že člověk často potřebuje start. Silný impuls a dobití baterek, aby prostě začal a hlavně — nepřestal, pokračoval ve zdokonalování a stal se tím, kým chce být. Je těžké být produktivní, když chybí základní síla.",
        "en": "We understand that people often need a head start. A powerful impulse and recharge just to begin, and more importantly, not to quit, but to keep improving and become the person they want to be. It's hard to be productive when you lack baseline energy."
    },
    "life_h2_2": {
        "ru": "Точка запуска",
        "ua": "Точка запуску",
        "cz": "Výchozí bod",
        "en": "The Launch Point"
    },
    "life_p_2": {
        "ru": "Чтобы появилась та самая искра, нужно начать с заботы о себе. Когда мы очищаем внутреннюю среду, наше тело становится чистым и насыщенным, а ум — осознанным и ясным. Именно отсюда берется сила, концентрация и креатив для достижения результатов.",
        "ua": "Щоб з'явилася та сама іскра, потрібно почати з турботи про себе. Коли ми очищаємо внутрішнє середовище, наше тіло стає чистим і насиченим, а розум — усвідомленим і ясним. Саме звідси береться сила, концентрація і креатив для досягнення результатів.",
        "cz": "Aby se objevila ta jiskra, je třeba začít péčí o sebe. Když pročistíme vnitřní prostředí, naše tělo se stane čistým a vyživeným, zatímco mysl je uvědomělá a jasná. Právě odtud pramení síla, soustředění a kreativita pro dosahování výsledků.",
        "en": "For that spark to ignite, you must start with self-care. When we cleanse our internal environment, our body becomes pure and nourished, while our mind becomes clear and conscious. This is where strength, concentration, and creativity come from to achieve results."
    },
    "life_insight": {
        "ru": "В чистом теле вредные привычки отпадают сами собой. А это дает нам колоссальное количество свободного времени, энергии и новых возможностей просто быть собой!",
        "ua": "У чистому тілі шкідливі звички відпадають самі собою. А це дає нам колосальну кількість вільного часу, енергії та нових можливостей просто бути собою!",
        "cz": "V čistém těle špatné návyky mizí samy od sebe. A to nám dává obrovské množství volného času, energie a nových příležitostí být prostě sami sebou!",
        "en": "In a clean body, bad habits fall away naturally. This frees up an immense amount of time, energy, and new opportunities to simply be yourself!"
    },
    "life_h2_3": {
        "ru": "Ваша «Дорога» по 16 жизненным сферам",
        "ua": "Ваша «Дорога» по 16 життєвим сферам",
        "cz": "Vaše „Cesta“ 16 sférami života",
        "en": "Your 'Journey' Across 16 Spheres of Life"
    },
    "life_p_3": {
        "ru": "Чтобы вам было легко идти по этому пути, мы создали инспиративный и информативный гайд — вашу дорогу по 16 жизненным сферам:",
        "ua": "Щоб вам було легко йти цим шляхом, ми створили інспіративний та інформативний гайд — вашу дорогу по 16 життєвим сферам:",
        "cz": "Aby se vám po této cestě kráčelo snadněji, vytvořili jsme inspirativního a informativního průvodce — vaši cestu přes 16 životních sfér:",
        "en": "To make this journey easy for you, we have created an inspirational and informative guide — your map across the 16 spheres of life:"
    },
    "life_sph_1_h": { "ru": "Физика и Энергия", "ua": "Фізика та Енергія", "cz": "Fyzika a Energie", "en": "Physics and Energy" },
    "life_sph_1_p": {
        "ru": "База для вашей «батарейки» — тело, сон, восстановление.",
        "ua": "База для вашої «батарейки» — тіло, сон, відновлення.",
        "cz": "Základ pro vaši „baterii“ — tělo, spánek, regenerace.",
        "en": "The foundation of your 'battery' — body, sleep, recovery."
    },
    "life_sph_2_h": { "ru": "Разум и Эмоции", "ua": "Розум та Емоції", "cz": "Mysl a Emoce", "en": "Mind and Emotions" },
    "life_sph_2_p": {
        "ru": "Ваш внутренний мир — когнитивная гигиена, творчество, радость.",
        "ua": "Ваш внутрішній світ — когнітивна гігієна, творчість, радість.",
        "cz": "Váš vnitřní svět — kognitivní hygiena, kreativita, radost.",
        "en": "Your inner world — cognitive hygiene, creativity, joy."
    },
    "life_sph_3_h": { "ru": "Системы и Ресурсы", "ua": "Системи та Ресурси", "cz": "Systémy a Zdroje", "en": "Systems and Resources" },
    "life_sph_3_p": {
        "ru": "Ваша материальная опора — финансы, время, цифровой баланс.",
        "ua": "Ваша матеріальна опора — фінанси, час, цифровий баланс.",
        "cz": "Vaše materiální opora — finance, čas, digitální rovnováha.",
        "en": "Your material foundation — finances, time, digital balance."
    },
    "life_sph_4_h": { "ru": "Социум и Проявление", "ua": "Соціум та Проявлення", "cz": "Společnost a Projev", "en": "Society and Expression" },
    "life_sph_4_p": {
        "ru": "Взаимодействие с миром — отношения, реализация, пространство.",
        "ua": "Взаємодія зі світом — відносини, реалізація, простір.",
        "cz": "Interakce se světem — vztahy, seberealizace, prostor.",
        "en": "Interacting with the world — relationships, realization, environment."
    },
    "life_sph_5_h": { "ru": "Смыслы", "ua": "Сенси", "cz": "Smysl", "en": "Meaning" },
    "life_sph_5_p": {
        "ru": "Ваш вектор — философия и масштаб личности.",
        "ua": "Ваш вектор — філософія і масштаб особистості.",
        "cz": "Váš směr — filozofie a velikost osobnosti.",
        "en": "Your trajectory — philosophy and personal scale."
    },
    "life_btn_pdf": {
        "ru": "Получить премиальный PDF-гайд «Карта Жизни»",
        "ua": "Отримати преміальний PDF-гайд «Карта Життя»",
        "cz": "Získat prémiového PDF průvodce „Mapa Života“",
        "en": "Get the Premium 'Life Map' PDF Guide"
    },
    "life_h2_4": {
        "ru": "Идем вместе",
        "ua": "Йдемо разом",
        "cz": "Jdeme společně",
        "en": "Walking Together"
    },
    "life_p_4": {
        "ru": "Идти по пути целостного саморазвития — это не только про здоровье, это про новые горизонты. Это возможность посоветовать этот путь друзьям и близким, вместе открывать новые активности и интересы. Это приведет к новым знакомствам, расширению ваших способностей и выведет качество жизни на совершенно другой уровень.",
        "ua": "Йти шляхом цілісного саморозвитку — це не тільки про здоров'я, це про нові горизонти. Це можливість порадити цей шлях друзям і близьким, разом відкривати нові активності та інтереси. Це призведе до нових знайомств, розширення ваших здібностей і виведе якість життя на абсолютно інший рівень.",
        "cz": "Kráčet po cestě celostního seberozvoje není jen o zdraví, ale o nových horizontech. Je to příležitost doporučit tuto cestu přátelům a rodině, objevovat společně nové aktivity a zájmy. To povede k novým známostem, rozšíření vašich schopností a posune kvalitu vašeho života na zcela novou úroveň.",
        "en": "Walking the path of holistic self-development is not just about health; it's about new horizons. It’s an opportunity to recommend this journey to friends and loved ones, to discover new activities and interests together. This will lead to new connections, expand your abilities, and elevate your quality of life to an entirely new level."
    },

    // ════════════ TOOLS (PANEL 4) ════════════
    "tools_badge": {
        "ru": "Практика и Восстановление",
        "ua": "Практика та Відновлення",
        "cz": "Praxe a Regenerace",
        "en": "Practice & Recovery"
    },
    "tools_title": {
        "ru": "Природа, усиленная наукой",
        "ua": "Природа, посилена наукою",
        "cz": "Příroda posílená vědou",
        "en": "Nature, Empowered by Science"
    },
    "tools_sub": {
        "ru": "Ваш арсенал восстановления. Мы создали комплексный набор, где каждый элемент усиливает действие другого. Это не просто добавки — это последовательная система очищения, питания и защиты вашего организма.",
        "ua": "Ваш арсенал відновлення. Ми створили комплексний набір, де кожен елемент посилює дію іншого. Це не просто добавки — це послідовна система очищення, живлення та захисту вашого організму.",
        "cz": "Váš arzenál pro obnovu. Vytvořili jsme komplexní sadu, kde každý prvek zesiluje účinek dalšího. Nejsou to jen doplňky — je to ucelený systém čištění, výživy a ochrany vašeho těla.",
        "en": "Your recovery arsenal. We have created a comprehensive kit where each element amplifies the other. These aren't just supplements — it is a sequential system for cleansing, nourishing, and protecting your body."
    },
    "tools_prod_1_h": { "ru": "Горькие травы", "ua": "Гіркі трави", "cz": "Hořké byliny", "en": "Bitter Herbs" },
    "tools_prod_1_p": {
        "ru": "Базовый антипаразитарный этап. Мягкая нейтрализация патогенов и грибков без агрессии для ЖКТ.",
        "ua": "Базовий антипаразитарний етап. М'яка нейтралізація патогенів та грибків без агресії для ШКТ.",
        "cz": "Základní antiparazitární fáze. Jemná neutralizace patogenů a plísní bez poškození trávicího traktu.",
        "en": "The foundational anti-parasitic stage. Gentle neutralization of pathogens and fungi without harming the GI tract."
    },
    "tools_prod_1_btn": { "ru": "Подробнее о составе", "ua": "Детальніше про склад", "cz": "Více o složení", "en": "Details on Formula" },
    "tools_prod_2_h": { "ru": "Настойка ПВЖМ", "ua": "Настоянка ПВВМ", "cz": "Tinktura PVM", "en": "Wax Moth Tincture" },
    "tools_prod_2_p": {
        "ru": "Утренняя поддержка. Уникальный фермент, способный растворять защитные оболочки паразитов и вирусов.",
        "ua": "Ранкова підтримка. Унікальний фермент, здатний розчиняти захисні оболонки паразитів та вірусів.",
        "cz": "Ranní podpora. Unikátní enzym schopný rozpouštět ochranné obaly parazitů a virů.",
        "en": "Morning support. A unique enzyme capable of dissolving the protective biofilms of parasites and viruses."
    },
    "tools_prod_2_btn": { "ru": "Подробнее о свойствах", "ua": "Детальніше про властивості", "cz": "Více o vlastnostech", "en": "Details on Properties" },
    "tools_prod_3_h": { "ru": "Живые ферменты", "ua": "Живі ферменти", "cz": "Živé enzymy", "en": "Live Enzymes" },
    "tools_prod_3_p": {
        "ru": "Подготовка к пищеварению. Яблочный уксус для нормализации кислотности и качественного усвоения микроэлементов.",
        "ua": "Підготовка до травлення. Яблучний оцет для нормалізації кислотності та якісного засвоєння мікроелементів.",
        "cz": "Příprava na trávení. Jablečný ocet pro normalizaci kyselosti a efektivní vstřebávání mikroživin.",
        "en": "Digestion prep. Apple cider vinegar to normalize acidity and ensure quality absorption of micronutrients."
    },
    "tools_prod_3_btn": { "ru": "Подробнее", "ua": "Детальніше", "cz": "Více", "en": "Details" },
    "tools_prod_4_h": { "ru": "Травяной чай «45 трав»", "ua": "Трав'яний чай «45 трав»", "cz": "Bylinný čaj „45 bylin“", "en": "'45 Herbs' Tea" },
    "tools_prod_4_p": {
        "ru": "Глубокое питание и детокс. Пьется в течение дня. Очищает лимфу, питает клетки и идеально подходит для процедуры тюбажа.",
        "ua": "Глибоке живлення та детокс. П'ється протягом дня. Очищає лімфу, живить клітини і ідеально підходить для процедури тюбажу.",
        "cz": "Hluboká výživa a detoxikace. Pije se během dne. Čistí lymfu, vyživuje buňky a je ideální pro proceduru očisty jater (tubage).",
        "en": "Deep nourishment and detox. Sipped throughout the day. Cleanses the lymph, feeds cells, and is perfect for the tubage procedure."
    },
    "tools_prod_4_btn": { "ru": "Подробнее о сборе", "ua": "Детальніше про збір", "cz": "Více o směsi", "en": "Details on Blend" },
    "tools_prod_5_h": { "ru": "Умные сорбенты", "ua": "Розумні сорбенти", "cz": "Chytré sorbenty", "en": "Smart Sorbents" },
    "tools_prod_5_p": {
        "ru": "Безопасная эвакуация токсинов. Работают как природная губка, связывая и выводя продукты распада без вреда для слизистой.",
        "ua": "Безпечна евакуація токсинів. Працюють як природна губка, зв'язуючи і виводячи продукти розпаду без шкоди для слизової.",
        "cz": "Bezpečná evakuace toxinů. Fungují jako přírodní houba, která váže a vylučuje odpadní látky, aniž by poškozovala sliznici.",
        "en": "Safe extraction of toxins. Acting as a natural sponge, they bind and expel waste products without harming the mucosal lining."
    },
    "tools_prod_5_btn": { "ru": "Как они работают", "ua": "Як вони працюють", "cz": "Jak fungují", "en": "How They Work" },
    "tools_rules_h2": {
        "ru": "Как это работает: Три золотых правила курса.",
        "ua": "Як це працює: Три золотих правила курсу.",
        "cz": "Jak to funguje: Tři zlatá pravidla kurzu.",
        "en": "How It Works: Three Golden Rules."
    },
    "tools_rules_p": {
        "ru": "Полный протокол вы найдете в нашем PDF-гайде, но основа успеха держится на трех китах:",
        "ua": "Повний протокол ви знайдете в нашому PDF-гайді, але основа успіху тримається на трьох китах:",
        "cz": "Kompletní protokol najdete v našem PDF průvodci, ale základ úspěchu stojí na třech pilířích:",
        "en": "The full protocol is available in our PDF guide, but the foundation of success rests on three pillars:"
    },
    "tools_rule_1_h": { "ru": "Чистое топливо (Диета)", "ua": "Чисте паливо (Дієта)", "cz": "Čisté palivo (Dieta)", "en": "Clean Fuel (Diet)" },
    "tools_rule_1_p": {
        "ru": "Убираем «пищевой шум» (сладкое, мучное, рафинированную еду). Основу рациона составляют овощи (квашеная капуста), зелень, кислые фрукты (грейпфрут, киви, ягоды). Главное — не переедать и ужинать за 3 часа до сна!",
        "ua": "Прибираємо «харчовий шум» (солодке, борошняне, рафіновану їжу). Основу раціону складають овочі (квашена капуста), зелень, кислі фрукти (грейпфрут, ківі, ягоди). Головне — не переїдати і вечеряти за 3 години до сну!",
        "cz": "Odstraňujeme „potravinový šum“ (sladkosti, mouku, rafinované potraviny). Základ stravy tvoří zelenina (kysané zelí), bylinky a kyselé ovoce (grep, kiwi, bobule). Hlavní je se nepřejídat a večeřet 3 hodiny před spaním!",
        "en": "Remove 'food noise' (sweets, baked goods, refined foods). The diet relies on vegetables (sauerkraut), greens, and tart fruits (grapefruit, kiwi, berries). Crucially — do not overeat, and finish dinner 3 hours before bed!"
    },
    "tools_rule_2_h": { "ru": "Промывание системы", "ua": "Промивання системи", "cz": "Proplachování systému", "en": "Flushing the System" },
    "tools_rule_2_p": {
        "ru": "Вода — ваш главный помощник. При дискомфорте или легком ознобе (нормальная реакция очищения), пейте больше чистой воды и используйте натуральные абсорбенты.",
        "ua": "Вода — ваш головний помічник. При дискомфорті або легкому ознобі (нормальна реакція очищення), пийте більше чистої води та використовуйте натуральні абсорбенти.",
        "cz": "Voda je váš hlavní pomocník. Při potížích nebo mírném zimnici (normální očistná reakce) pijte více čisté vody a použijte přírodní absorbenty.",
        "en": "Water is your primary ally. In case of discomfort or slight chills (a normal detox reaction), drink plenty of clean water and use natural absorbents."
    },
    "tools_rule_3_h": { "ru": "Тюбаж печени (Глубокая очистка)", "ua": "Тюбаж печінки (Глибоке очищення)", "cz": "Tubage Jater (Hluboká očista)", "en": "Liver Tubage (Deep Cleanse)" },
    "tools_rule_3_p": {
        "ru": "Мощная, опциональная процедура для высвобождения застойной желчи. Выполняется с помощью магнезии, теплой грелки на правый бок и сбора «45 трав».",
        "ua": "Потужна, опціональна процедура для вивільнення застійної жовчі. Виконується за допомогою магнезії, теплої грілки на правий бік і збору «45 трав».",
        "cz": "Silná, volitelná procedura k uvolnění stagnující žluči. Provádí se pomocí magnézia, teplého termoforu na pravý bok a bylinné směsi „45 bylin“.",
        "en": "A powerful, optional procedure to release stagnant bile. It is performed using magnesia, a warm heating pad on the right side, and the '45 Herbs' blend."
    },
    "tools_inst_h2": { "ru": "Инструкции", "ua": "Інструкції", "cz": "Instrukce", "en": "Instructions" },
    "tools_inst_vid_h": { "ru": "Смотреть инструкцию", "ua": "Дивитися інструкцію", "cz": "Sledovat návod", "en": "Watch Instructions" },
    "tools_inst_vid_label": { "ru": "Видео-инструкция", "ua": "Відео-інструкція", "cz": "Video návod", "en": "Video Tutorial" },
    "tools_inst_vid_p": {
        "ru": "Посмотрите короткое видео, где мы наглядно показываем, как правильно разводить сорбенты, заваривать чай и в какой последовательности принимать настойки.",
        "ua": "Подивіться коротке відео, де ми наочно показуємо, як правильно розводити сорбенти, заварювати чай і в якій послідовності приймати настоянки.",
        "cz": "Podívejte se na krátké video, kde jasně ukazujeme, jak správně ředit sorbenty, vařit čaj a v jakém pořadí užívat tinktury.",
        "en": "Watch a short video where we clearly demonstrate how to correctly mix sorbents, brew the tea, and the sequence in which to take the tinctures."
    },
    "tools_inst_pdf_h": { "ru": "Полный протокол в вашем смартфоне", "ua": "Повний протокол у вашому смартфоні", "cz": "Kompletní protokol ve vašem telefonu", "en": "Full Protocol in Your Phone" },
    "tools_inst_pdf_p": { "ru": "Мы собрали все детали в одном месте:", "ua": "Ми зібрали всі деталі в одному місці:", "cz": "Shromáždili jsme všechny detaily na jedno místo:", "en": "We gathered all the details in one place:" },
    "tools_inst_pdf_li1": { "ru": "Точные дозировки и тайминги приема (утро/день/вечер)", "ua": "Точні дозування та таймінги прийому (ранок/день/вечір)", "cz": "Přesné dávkování a načasování (ráno/den/večer)", "en": "Exact dosages and timing (morning/noon/night)" },
    "tools_inst_pdf_li2": { "ru": "Подробное меню и списки продуктов", "ua": "Детальне меню та списки продуктів", "cz": "Podrobné menu a seznamy potravin", "en": "Detailed menu and grocery lists" },
    
    // The PDF button texts are handled separately via attributes, but we can put them here if we refactor HTML. Let's keep them here for safety.
    "btn_santiago_pdf": {
        "ru": "Скачать протокол Santiago (PDF)",
        "ua": "Завантажити протокол Santiago (PDF)",
        "cz": "Stáhnout protokol Santiago (PDF)",
        "en": "Download Santiago Protocol (PDF)"
    },
    "tools_tubazh_h4": {
        "ru": "Тюбаж печени — отдельная инструкция",
        "ua": "Тюбаж печінки — окрема інструкція",
        "cz": "Tubage Jater — samostatný návod",
        "en": "Liver Tubage — Separate Guide"
    },
    "tools_tubazh_p": {
        "ru": "Пошаговый алгоритм процедуры тюбажа для глубокой очистки печени и желчного пузыря.",
        "ua": "Покроковий алгоритм процедури тюбажу для глибокого очищення печінки та жовчного міхура.",
        "cz": "Průvodce krok za krokem procedurou tubage pro hlubokou očistu jater a žlučníku.",
        "en": "A step-by-step guide for the tubage procedure for deep liver and gallbladder cleansing."
    },
    "btn_tubazh_pdf": {
        "ru": "Скачать инструкцию Тюбажа (PDF)",
        "ua": "Завантажити інструкцію Тюбажу (PDF)",
        "cz": "Stáhnout návod Tubage (PDF)",
        "en": "Download Tubage Guide (PDF)"
    }
};

window.siteTranslations = translations;
